...
async def test_evaluate_retrieval_route(authenticated_client):
    ...
    ), patch("chat.adapters.inbound.http.evaluate_retrieval_internal", async_mock):
...
async def test_evaluate_answer_route(authenticated_client):
    ...
    async_mock = AsyncMock(return_value=(eval_result, generated_answer, chunks, retrieval))

    with patch(
        "chat.adapters.inbound.http.resolve_evaluation_tests", AsyncMock(return_value=sample)
    ), patch(
        "chat.adapters.inbound.http.get_user_tenant_id", AsyncMock(return_value="tenant-1")
    ), patch("chat.adapters.inbound.http.evaluate_answer_internal", async_mock):
...
def test_upload_tests_endpoint(authenticated_client, tmp_path):
    ...
    with patch("chat.adapters.inbound.http.TEST_FILE", destination), patch(
        "chat.adapters.inbound.http.get_user_tenant_id", AsyncMock(return_value="tenant-1")
    ), patch(
        "chat.adapters.inbound.http.init_retrieval_dataset_table", AsyncMock()
...
async def test_load_unified_tests_rollbacks_after_db_error():
    from chat.adapters.inbound.http import load_unified_tests
    ...
    with patch("chat.adapters.inbound.http.load_tests", return_value=sample), patch(
        "chat.adapters.inbound.http.init_retrieval_dataset_table",
...
async def test_load_unified_tests_merges_gold_jsonl_with_db_rows():
    ...
    with patch("chat.adapters.inbound.http.load_tests", return_value=gold), patch(
        "chat.adapters.inbound.http.init_retrieval_dataset_table",
        AsyncMock(),
    ):
        tests = await load_unified_tests(db, "11111111-1111-1111-1111-111111111111")
