...
from opendata.http import require_admin
...
def test_get_sat_authenticated(authenticated_client):
    with patch(
        "opendata.http.opendata_service.list_sat_societies",
...
def test_import_requires_admin(authenticated_client):
    with patch("opendata.http.user_is_admin", new=AsyncMock(return_value=False)):
...
def test_import_as_admin(authenticated_client):
    ...
    try:
        with patch(
            "opendata.http.opendata_service.import_opendata_directory",
            new=AsyncMock(
                return_value={
...
def test_istac_series_and_observations(authenticated_client):
    with patch(
        "opendata.http.opendata_service.list_istac_series",
    ...
    with patch(
        "opendata.http.opendata_service.list_istac_observations",
