...
import asyncio
from services.chat_progress import sse_encode
...
from chat.postgres import build_chat_container
from identity.http import http_error
...
from utils.scope import get_user_tenant_id
...
from evaluation.ragas import RAGAS_METRICS, RagasSample, run_ragas_evaluation
from evaluation.hallucination import decide_is_hallucination
from services.embedding_indexer import get_active_runtime_model
...
from services.agent_modes import resolve_chat_modes
...
def _chunk_texts(docs) -> list[str]:
    texts: list[str] = []
    for doc in docs or []:
        text = (
            getattr(doc, "page_content", None)
...
def _ragas_fallback_evaluator(records: list[dict]) -> list[dict]:
    """Fallback determinista si la librería ragas no está operativa."""
    rows = []
    for record in records:
        prompt = set(str(record.get("user_input") or "").lower().split())
        answer = set(str(record.get("response") or "").lower().split())
        contexts = record.get("retrieved_contexts") or []
        context = set(" ".join(str(c) for c in contexts).lower().split())
        reference = set(str(record.get("reference") or "").lower().split())
        overlap_context = len(answer & context) / max(1, len(answer)) if answer else 0.0
        relevance = len(answer & prompt) / max(1, len(prompt)) if prompt else 0.0
        recall = (
            len(context & reference) / max(1, len(reference)) if reference else 0.0
        )
        rows.append(
            {
                **record,
...
def _score_ragas_sample(
    ...
    """Ejecuta RAGAS (o fallback) sobre una muestra y devuelve métricas + origen."""
    sample = RagasSample(
        user_input=question,
    ...
    source = "ragas"
    try:
        result = run_ragas_evaluation([sample])
    except Exception:
        result = run_ragas_evaluation([sample], evaluator=_ragas_fallback_evaluator)
        source = "ragas_fallback"
    row = (result.get("rows") or [{}])[0]
    metrics = {
        metric: (
            float(row[metric])
            if row.get(metric) is not None and str(row.get(metric)).strip() != ""
            else None
        )
...
async def evaluate_answer_internal(
    ...
    """Genera y juzga respuesta con RAG y sin RAG; añade métricas RAGAS."""
    evaluator = RAGService(
    ...
    reference = getattr(test, "reference_answer", None)
    ...
    context_texts = _chunk_texts(retrieved_docs)
    ...
    try:
        vanilla_resp = await evaluator.simple_chat(test.question, temperature=temp)
        vanilla_answer = vanilla_resp.get("answer", "") or ""
    except Exception:
        vanilla_answer = ""

    try:
        vanilla_eval, _, _ = await evaluator.evaluate_answer(
            test.question,
            vanilla_answer,
            [],
            reference_answer=reference,
    ...
    rag_ragas, ragas_source = _score_ragas_sample(
        test.question, generated_answer or "", context_texts, reference
    )
    vanilla_ragas, vanilla_source = _score_ragas_sample(
        test.question, vanilla_answer or "", [], reference
    )
    # Si alguno cayó a fallback, etiquetar el lote como fallback.
    if vanilla_source != "ragas" or ragas_source != "ragas":
        ragas_source = "ragas_fallback"

    rag_hall = decide_is_hallucination(
        answer=generated_answer or "",
        contexts=context_texts,
        faithfulness=rag_ragas.get("faithfulness"),
    )
    vanilla_hall = decide_is_hallucination(
        answer=vanilla_answer or "",
        contexts=[],
        faithfulness=vanilla_ragas.get("faithfulness"),
    )
    ...
    return (
        ...
        {
            "ragas": rag_ragas,
            "vanilla_ragas": vanilla_ragas,
            "ragas_source": ragas_source,
            "is_hallucination": rag_hall.get("is_hallucination"),
            "vanilla_is_hallucination": vanilla_hall.get("is_hallucination"),
        },
    )
...
async def load_unified_tests(
    ...
    *,
    ...
    Por defecto excluye preguntas HITL pendientes (``validated=false``) para
    que la auditoría no mezcle basura de extracciones automáticas.
    """
    ...
    for item in tests:
        ...
        if not validated_only:
            curated.append(item)
            continue
        # Solo HITL/document aprobados (validated debe ser True explícito).
        if source in {"hitl", "document"}:
...
async def _indexed_embedding_models(db: AsyncSession) -> list[str]:
    try:
        result = await db.execute(
            text(
                """
...
async def list_indexed_embedding_models(
    ...
    indexed = await _indexed_embedding_models(db)
...
@lab_router.post(
    "/evaluation/experiments/indexed-models",
    response_model=ExperimentCompareResponse,
)
async def run_indexed_embedding_experiments(
    request: ExperimentRunRequest,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    organization_id: UUID | None = None,
):
    """Una corrida del banco de oro por cada modelo con vectores indexados."""
    tenant_id = await get_user_tenant_id(
        current_user.id, db, organization_id=organization_id
    )
    tests = await resolve_evaluation_tests(
    ...
    if request.embedding_models:
        ...
        models = await _indexed_embedding_models(db)
    if not models:
        models = [request.embedding_model or DEFAULT_EMBEDDING_MODEL]
    collections = [str(request.knowledge_base_id)] if request.knowledge_base_id else None
    ...
    for embedding_model in models:
        ...
        metrics.setdefault("parameters", {})["temperature"] = request.temperature
        metrics.setdefault("parameters", {})["indexed_models"] = models
        row = {
            "id": 0,
            "created_at": datetime.now(timezone.utc),
            "generation_model": None,
        ...
        if request.persist:
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"No se pudo guardar la corrida de {embedding_model}: {exc}",
                ) from exc
    ...
    return ExperimentCompareResponse(
        ...
        note=(
            "Una corrida del banco de oro por cada modelo con embeddings "
            f"en el índice ({len(runs)} modelo(s))."
        ),
    )
...
@router.get("/conversations/user-history")
async def get_user_conversation_history(
    ...
    """Historial de chats del tenant: memoria a largo plazo de conversaciones."""
    tenant_id = await get_user_tenant_id(
        current_user.id, db, organization_id=organization_id
    )
    user_filter = str(current_user.id) if mine_only else user_id
    conv = build_chat_container(db).conversations
    items = await conv.list_user_history_for_tenant(
        tenant_id,
    ...
    for item in items:
        ...
        bucket = users.setdefault(
            ...
            {
                "user_id": uid,
                "user_name": item.get("user_name") or "Usuario",
    ...
    return {
        ...
        "conversations": items,
        "note": (
            "Memoria a largo plazo del chat: conversaciones y mensajes persistidos "
            "en PostgreSQL. La memoria a corto plazo es el historial reciente "
            "inyectado en el prompt (2–6 turnos)."
        ),
    }
...
async def get_conversation_detail_for_tenant(
    ...
    """Detalle de mensajes de una conversación del tenant (lectura org)."""
    tenant_id = await get_user_tenant_id(
        current_user.id, db, organization_id=organization_id
    )
    try:
        return await build_chat_container(db).conversations.get_with_messages_for_tenant(
...
async def get_evaluation_runtime_config(
    ...
    _ = current_user
    try:
        embedding_model = await get_active_runtime_model(db)
    except Exception:
        embedding_model = DEFAULT_EMBEDDING_MODEL
    return rag_runtime_config(embedding_model=embedding_model)
...
async def get_evaluation_tests(
    ...
    tests = await resolve_evaluation_tests(
        ...
        db,
        organization_id,
        validated_only=not include_pending,
    )
    return {
        "total": len(tests),
...
@lab_router.post("/evaluation/purge-pending-tests")
async def purge_pending_evaluation_tests(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    organization_id: UUID | None = None,
):
    """Elimina HITL no validadas y plantillas genéricas del banco DB."""
    tenant_id = await get_user_tenant_id(
        current_user.id, db, organization_id=organization_id
    )
    await init_retrieval_dataset_table(db)
    result = await db.execute(
        text("""
            ...
            WHERE tenant_id = :tenant_id
              AND (
                (
                  COALESCE(metadata->>'source', '') IN ('hitl', 'document')
                  AND COALESCE(metadata->>'validated', 'false') <> 'true'
                )
                OR COALESCE(reference_answer, '') ILIKE
                    '%Pregunta de evaluación derivada del documento%'
                OR question ILIKE '¿Cuál es el hecho principal descrito en %'
                OR question ILIKE '¿Qué fechas o plazos relevantes establece %'
                OR question ILIKE '¿Qué relación existe entre los elementos principales de %'
                OR question ILIKE '¿Qué procedimiento describe %'
                OR question ILIKE '¿Qué requisitos de cumplimiento establece %'
                OR question ILIKE '¿Qué dato normativo concreto recoge %'
              )
            RETURNING id
        """),
        {"tenant_id": tenant_id},
    )
    deleted_bank = len(result.fetchall())

    # También limpia la cola de aprobación HITL.
    deleted_hitl = 0
    try:
        ...
        deleted_hitl = len(hitl.fetchall())
    except Exception:
        await safe_rollback(db)
        # Re-init retrieval commit path below still needed
        await init_retrieval_dataset_table(db)

    await db.commit()
    return {
        "status": "ok",
        "deleted": deleted_bank + deleted_hitl,
        "deleted_bank": deleted_bank,
        "deleted_hitl": deleted_hitl,
    }


@lab_router.post("/evaluation/reset-to-gold")
async def reset_evaluation_bank_to_gold(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    organization_id: UUID | None = None,
):
    """Vacía el banco DB del tenant y reinserta solo el JSONL de oro actual."""
    tenant_id = await get_user_tenant_id(
        current_user.id, db, organization_id=organization_id
    )
    gold = load_tests()
    if not gold:
        raise HTTPException(status_code=400, detail="El banco de oro local está vacío")
        ...
        await db.execute(
            text("""
                INSERT INTO public.retrieval_dataset (
                    ...
                    FALSE, :out_of_knowledge, :split, CAST(:metadata AS JSONB)
                )
            """),
            {
                "tenant_id": tenant_id,
                "question": item.question.strip(),
                "keywords": json.dumps(list(item.keywords or []), ensure_ascii=False),
...
async def chat(
    ...
    try:
        ...
        rag_mode_used = "none"
        try:
            corpus_embed = await get_active_runtime_model(db)
        except Exception:
            corpus_embed = DEFAULT_EMBEDDING_MODEL
        core_rag = RAGService(
            model=selected_model,
            embedding_model=corpus_embed,
            **chat_service_ks(request.retrieval_k, request.final_k),
        )
        ...
        if not request.use_rag:
            ...
            if mode == "hybrid":
            ...
            elif mode == "compare" or agent_mode == "multitask":
                hybrid_result, agentic_result = await asyncio.gather(
                    run_hybrid_answer(core_rag, **hybrid_kwargs),
                    agentic.answer(
                        request.question,
                        tenant_id=str(tenant_id),
                        collections=collections,
                ...
                comparison_payload = {
                    ...
                    "note": (
                        "Multitask: mismo corpus y pregunta. Hybrid = pipeline "
                        "fijo denso+BM25+RRF. Agent = LangGraph con herramientas."
                    ),
                }
        ...
        return ChatResponse(
            ...
            agent_mode=rag_result.get("agent_mode"),
...
def _context_chunks_from_rag(chunks) -> list[ContextChunk]:
    contextos: list[ContextChunk] = []
...
async def chat_stream(
    ...
    """SSE: pasos del agente + tokens de respuesta en tiempo real."""
    ...
    async def run_pipeline() -> None:
        try:
            ...
            if not request.use_rag:
                ...
                if answer_text:
                    await on_progress({"type": "token", "delta": answer_text})
            else:
                agentic = AgenticRAGService(rag=core_rag, on_progress=on_progress)
                resolved = resolve_chat_modes(
                    agent_mode=getattr(request, "agent_mode", None),
                    rag_mode=request.rag_mode,
                )
                ...
                if mode == "hybrid":
                    ...
                    # Multitask: progreso del agent; hybrid en paralelo silencioso.
                    hybrid_task = run_hybrid_answer(
                        core_rag,
                        question=request.question,
                        tenant_id=str(tenant_id),
                        collections=collections,
                    ...
                    comparison_payload = {
                        ...
                        "note": (
                            "Multitask: mismo corpus y pregunta. Hybrid = pipeline "
                            "fijo; Agent = LangGraph con herramientas."
                        ),
                    }
                    rag_result = agentic_result
                    rag_mode_used = "compare"
                    agent_mode_used = "multitask"
                else:
                    rag_result = await agentic.answer(
                        request.question,
                        tenant_id=str(tenant_id),
                ...
                architecture = rag_result.get("architecture")
                agent_trace = rag_result.get("agent_trace")
                rag_result["agent_mode"] = agent_mode_used

            answer = rag_result["answer"]
            chunks = rag_result["chunks"]
            retrieval = rag_result["retrieval"]
            if agent_mode_used is None and rag_mode_used not in {"none", "hybrid", "compare"}:
                agent_mode_used = rag_mode_used
            ...
            contextos = _context_chunks_from_rag(chunks)
            ...
            try:
                ...
                if review_id:
                    await queue.put({"type": "review", "review_id": review_id})
            except Exception:
                pass
        except AppError as exc:
            await db.rollback()
            ...
            if "not found" in err.lower() and "model" in err.lower():
                detail = (
                    "Ollama no tiene el modelo. Descárgalo con ollama pull."
                )
            else:
...
async def evaluate_answer_route(
    ...
    # Ejecutar evaluación de generación: con RAG y sin RAG (vanilla) + RAGAS
    (
        eval_result,
        generated_answer,
        chunks,
        ...
        vanilla_answer,
        ragas_bundle,
    ) = await evaluate_answer_internal(
        test,
        str(tenant_id),
        collections=collections,
    ...
    eval_data = _to_dict(eval_result)
    vanilla_data = _to_dict(vanilla_eval)
    retrieval_data = _to_dict(retrieval)
    ragas_bundle = ragas_bundle or {}
    ...
    return {
        ...
        "vanilla_evaluation": vanilla_data,
        "ragas": ragas_bundle.get("ragas"),
        "vanilla_ragas": ragas_bundle.get("vanilla_ragas"),
        "ragas_source": ragas_bundle.get("ragas_source"),
        "is_hallucination": ragas_bundle.get("is_hallucination"),
        "vanilla_is_hallucination": ragas_bundle.get("vanilla_is_hallucination"),
        "comparison_mode": "no_rag_vs_rag",
        "retrieval": retrieval_data,
        "retrieved_chunks": [
            {
    ...
    async def event_generator():
        ...
        for idx, test in enumerate(tests):
            ...
            (
                ...
                _vanilla_answer,
                _ragas_bundle,
            ) = await evaluate_answer_internal(test, str(tenant_id))
