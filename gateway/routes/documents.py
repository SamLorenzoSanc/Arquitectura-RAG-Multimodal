...
from services.question_extraction import extract_document_questions
...
async def upload_document(
    ...
    """Ingesta local síncrona: guarda el fichero y registra metadatos + job."""
    await _validate_kb_access(db, knowledge_base_id, current_user)
    kb = await db.scalar(
        select(KnowledgeBase).where(KnowledgeBase.id == knowledge_base_id)
    )
    ...
    if not save_error:
        try:
            ...
            extracted = await extract_document_questions(
                document.filename, content, document.mime_type
            )
            if extracted:
                org_id = None
                tenant = await db.scalar(select(Tenant).where(Tenant.id == tenant_id))
                if tenant is not None:
                    org_id = str(getattr(tenant, "organization_id", "") or "") or None
    ...
    return {
        ...
        "questions": extracted,
        "message": (
            f"Error al procesar el documento: {job.error_message}"
            if failed
            else (
                f"Documento almacenado. Se enviaron {len(extracted)} preguntas de muestra a Validación humana."
                if extracted
                else "Documento almacenado. No se pudieron extraer preguntas de muestra."
            )
        ),
    }
