...
\midrule
Naive RAG & Recuperar y generar en un paso & Base del chat híbrido \\
Graph RAG & Entidades y aristas; multi-salto & Solo visualización UMAP; no se usa en el chat \\
Agentic RAG & Planificación y herramientas & Grafo LangGraph sobre el híbrido (recuperar o responder, grade, rewrite) \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Métricas de evaluación en sistemas RAG: definiciones de la literatura y lo que mide este prototipo}
La evaluación de arquitecturas RAG requiere métricas que consideren tanto la recuperación documental como la generación. El marco RAGAS (Retrieval-Augmented Generation Assessment; Es et al., 2024) propone, en la literatura, métricas de coherencia factual y relevancia que pueden calcularse con un LLM juez y, en algunos casos, sin respuesta de referencia. \textbf{En este TFM esas definiciones se recogen como estado del arte}; las cifras del Capítulo~\ref{ch:evaluacion} no deben leerse como un informe RAGAS oficial. El prototipo implementa tres familias: funciones propias de IR, un LLM-as-a-Judge local (escala 1--5) y un adaptador RAGAS opcional con fallback heurístico.

A continuación se recuerdan las definiciones habituales (fidelidad, relevancia de respuesta, precisión y exhaustividad del contexto, tasa de alucinaciones y calidad de citación), orientadas al dominio PAC/POSEI, para que el lector distinga qué se \emph{desea} medir y qué se \emph{calcula} realmente en el código.
