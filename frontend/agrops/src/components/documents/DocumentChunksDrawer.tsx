...
import type { DocumentChunk, DocumentItem, IndexState } from "@/types";
import { IndexStatesList } from "@/components/documents";
...
export default function DocumentChunksDrawer({
  ...
  const statesQuery = useQuery({
    queryKey: ["documents", document.id, "index-states"],
    queryFn: () =>
      DocumentService.indexStates({
        knowledgeBaseId: document.knowledge_base_id as string,
        documentId: document.id,
      }),
    enabled: Boolean(document.knowledge_base_id),
    refetchInterval: (current) => {
      const rows = current.state.data ?? [];
      return rows.some((state) => state.status === "pending" || state.status === "indexing")
        ? 4000
        : false;
    },
  });
  ...
  async function onRetryIndex(state: IndexState) {
    const kb = document.knowledge_base_id;
    if (!kb) return;
    setRetrying(state.id);
    try {
      await DocumentService.retryIndex({ knowledgeBaseId: kb, stateId: state.id });
      await queryClient.invalidateQueries({ queryKey: ["documents"] });
      await queryClient.invalidateQueries({
        queryKey: ["documents", document.id, "index-states"],
      });
  ...
  const indexStates = statesQuery.data ?? document.index_states ?? [];
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/30" onClick={onClose}>
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-auto px-5 py-4">
          ...
          {indexStates.length > 0 && (
            <div className="mb-4 rounded-xl border border-slate-100 bg-slate-50/80 p-3">
              <p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">
                {t("documentsDrawer.indexSection")}
              </p>
              <IndexStatesList
                states={indexStates}
                busyId={retrying}
                onRetry={(state) => void onRetryIndex(state)}
              />
            </div>
          )}
          {query.isLoading && (
