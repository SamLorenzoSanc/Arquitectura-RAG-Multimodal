import { useChromaDocuments } from "@/hooks/useCachedApi";
...
const ChromaDebugPage = () => {
  const { data, isLoading: loading, isError } = useChromaDocuments();
  const error = isError
    ? "Error al conectar con la base de datos de Chroma"
    : "";

  if (loading) {
    return (
      <div className="p-8 text-center">
        Cargando inspección de base de datos...
      </div>
    );
  }
  if (error) {
  ...
  const documentos = (data?.documentos ?? []) as Documento[];

  return (
    <div className="p-8 max-w-7xl mx-auto">
      <h1 className="text-3xl font-extrabold text-gray-900 mb-6">
        Inspección de ChromaDB
      </h1>
              ...
              documentos.map((doc) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 font-semibold text-blue-600">
                    {(doc.metadata?.tenant_id as string) || (
